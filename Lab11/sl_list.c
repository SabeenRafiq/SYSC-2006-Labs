...
intnode_t *add(intnode_t *head, int elem, int index)
{
    ...
    // If index is 0 (list empty or not)
    if (index == 0)
    {
    ...
    intnode_t *newnode = intnode_construct(elem, NULL);
...
int count_in_sll(intnode_t *head, int target)
{
    ...
    return counter + count_in_sll(head->next, target);
}
